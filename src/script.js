document.addEventListener("DOMContentLoaded", () => {
  setTimeout(async () => {
    ...
    muteElm.addEventListener("click", async () => {
      
      try {
      var player = document.getElementById('swarmfm');
      console.log("mute pressed, current mute status: "+ player.muted)
      if (player.muted) {
        muteElm
          .querySelector("path")
          .setAttribute(
        ...
        // for some reason we need to call player.play() when we changed if the stream is muted
        player.muted = false;
        await player.pause()
        await player.play();
